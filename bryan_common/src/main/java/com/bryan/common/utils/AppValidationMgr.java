...
public class AppValidationMgr {
    ...
    //小数表达式
    //^([1-9][0-9]*)+(.[0-9]{1,2})?$
//    private final static Pattern digital_pattern = Pattern.compile("-?[0-9]+.?[0-9]+");
    //非零开头的最多带两位小数的数字
    private final static Pattern digital_pattern = Pattern.compile("^([1-9][0-9]*)+(.[0-9]{1,2})?$");
...
//		 return phone_pattern.matcher(phone).matches();
        ...
        //联通
        String regex2 = "^((13[0-2])|(145)|(15[5-6])|(176)|(18[5,6]))\\\\d{8}|(1709)\\\\d{7}$";
        //电信
        String regex3 = "^((133)|(153)|(177)|(18[0,1,9]))\\\\d{8}$";
        return Pattern.matches(regex, phone) || Pattern.matches(regex2, phone) || Pattern.matches(regex3, phone);
    }
     ...
     * @return
     */
    public static Boolean isNumberPoints(String str) {
    ...
    public static String idHide(String id) {
        return id.replaceAll("(\\d{4})\\d{10}|\\d{9}|\\d{8}(\\d{4})", "$1**********$2");
    }
     ...
     * @return 星号替换的身份证号
     */
    public static String idHide2(String id) {
        if (id.length() > 8) {
